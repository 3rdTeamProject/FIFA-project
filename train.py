...
데이터 파싱/feature 조립은 preprocess.py, 저장은 utils.py에 맡기고, 여기서는
...
split에 쓰는 random seed를 고정하고 어떤 match_id/ouid가 train/validation/test 중 어디로
갔는지 output/split_assignment.json에 저장해둔다.

실사용/저장하는 모델은 sklearn Pipeline(StandardScaler + LogisticRegression)이다.
tier(2200~2400대)와 avg_stat_score(50~100대)는 값의 스케일 차이가 커서, 정규화가 걸리는
...
def main():
    ...
    utils.save_model(pipeline, f"{OUTPUT_DIR}/win_prediction_baseline_pipeline.joblib")
    utils.save_text(summary_text, f"{OUTPUT_DIR}/win_prediction_baseline_summary.txt")
    print(f"  [저장] 모델 -> {OUTPUT_DIR}/win_prediction_baseline_pipeline.joblib")
