...
def main():
    ...
    for i, (key, feature_columns, _) in enumerate(train.MODEL_SPECS, start=2):
        ...
        # train_style.py와 완전히 같은 방식(random_state=42 단일 실행 + FIXED_K_OVERRIDES)으로
        # 실제 채택 k를 구한다(이 스크립트 자체의 50-시드 평균 실루엣 1위 k와는 다를 수 있음 —
        # 요약문 참고). model_key를 넘겨야 shot_location의 override(k=2)가 반영된다.
        actual_k, _ = train.select_best_k(x_scaled, model_key=key)
